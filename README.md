...
1. [使用接口统一规范新旧方法并添加`@Janus`注解](#jump-target-interface-definition)。
...
Janus框架强制要求统一新旧方法的定义。详情见[接口定义](#jump-target-interface-definition)。
...
## <span id="jump-target-interface-definition">接口定义</span>
...
可以[自己注入该线程池](#jump-target-custom)，名字一样（必须使用`janusCompareThreadPool`这个名字）即可替换框架提供的线程池。
